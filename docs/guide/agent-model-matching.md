...
- **Kimi:** **K3** · K2.7
- **GLM:** 5.3-flash / 5.1 *(acceptable — slightly looser on the long nested workflows)*
...
**GLM remains lower-confidence than Claude/Kimi.** A GLM-calibrated prompt exists for model IDs recognized as GLM. The Sisyphus fallback chain no longer includes `glm-5.2` (superseded by the cheaper `glm-5.3-flash`); GLM remains a manual option.
...
> **GLM remains lower-confidence than Claude/Kimi.** It has a calibrated prompt but no maintainer end-to-end validation. The automatic chains no longer include `glm-5.2`; `glm-5.3-flash` is the cheap GLM rung, and older `glm-5` / `glm-5.1` entries are compatibility paths.
...
|---|---|---|---|
| **OpenCode Go** | $10/mo | `kimi-k3`, `glm-5.3-flash`, `minimax-m2.5`, `minimax-m2.7`, `minimax-m3`, `mimo-v2-pro`, `qwen3.7-plus`, `qwen3.6-plus` | Claude-family alternatives (Kimi, GLM), Gemini-family alternatives (Qwen), utility/retrieval (MiniMax) |
...
| 6 | **`glm-5.3-flash` — ACCEPTABLE CHEAP GLM OPTION** | `opencode-go`, `zai-coding-plan`, `vercel` | Claude-like, slightly looser on long nested workflows. GLM IDs use the calibrated GLM prompt; `glm-5.2` was removed from the automatic chains on cost. |
...
| 3 | `gpt-5.6-sol` (`medium`) | `openai`, `quotio-openai`, `github-copilot`, `opencode`, `vercel` | Final built-in visual fallback. |
...
`glm-5.2` was removed from the automatic fallback chains: `glm-5.3-flash` outperforms it at a fraction of the cost. Older `glm-5` / `glm-5.1` catalog entries remain compatibility paths for manual selection.
...
| --- | --- | --- |
| **sisyphus** | `claude-opus-5` | `anthropic\|github-copilot\|opencode\|vercel/claude-opus-5 (max)` → `opencode-go\|kimi-for-coding\|moonshotai\|opencode\|vercel\|bailian-coding-plan\|moonshotai-cn\|firmware\|ollama-cloud\|aihubmix/kimi-k3` → `openai\|github-copilot\|opencode\|vercel/gpt-5.6-sol (medium)` → `opencode/big-pickle` |
| **hephaestus** | `gpt-5.6-sol` | `openai\|github-copilot\|vercel\|opencode/gpt-5.6-sol (medium)` |
| **oracle** | `gpt-5.6-sol` | `openai\|opencode\|vercel/gpt-5.6-sol (xhigh)` → `github-copilot/gpt-5.6-sol (high)` → `google\|github-copilot\|opencode\|vercel/gemini-3.1-pro (high)` → `anthropic\|github-copilot\|opencode\|vercel/claude-opus-5 (max)` |
...
| **momus** | `gpt-5.6-terra` | `openai\|vercel/gpt-5.6-terra (high)` → `github-copilot/gpt-5.6-terra (high)` → `openai\|opencode\|vercel/gpt-5.6-sol (xhigh)` → `github-copilot/gpt-5.6-sol (high)` → `anthropic\|github-copilot\|opencode\|vercel/claude-opus-5 (max)` → `google\|github-copilot\|opencode\|vercel/gemini-3.1-pro (high)` |
...
| **GLM 5.2**           | Not selected by any automatic chain (`glm-5.3-flash` supersedes it on cost). Model IDs recognized as GLM still use the GLM-calibrated prompt on manual selection. |
...
| **opencode-go/glm-5.3-flash** | Text-only cheap GLM model. Used by the Manager fallback chain and cheap GLM rungs; supersedes `glm-5.2`.  |
...
Some model identifiers in fallback chains are provider-specific aliases. For example, `kimi-k3` resolves through `kimi-for-coding`, while `glm-5.3-flash` can resolve through `opencode-go`, `zai-coding-plan`, or `vercel` depending on availability.
...
| --- | --- | --- | --- |
| `visual-engineering` | Frontend, UI, CSS, design | `anthropic/claude-opus-5 (max)` | `anthropic\|anthropic-api\|github-copilot\|opencode\|vercel/claude-opus-5 (max)` → `kimi-for-coding\|moonshotai\|opencode-go\|opencode\|vercel/kimi-k3 (max)` → `openai\|quotio-openai\|github-copilot\|opencode\|vercel/gpt-5.6-sol (medium)` |
...
```jsonc
{
  "agents": {
    ...
    "oracle": { "model": "opencode-go/glm-5.3-flash" },  // Degraded but functional
    "explore": { "model": "opencode-go/qwen3.7-plus" },
    "librarian": { "model": "opencode-go/qwen3.7-plus" },
  },
  "categories": {
...
- Sisyphus: GLM 5.2. Model IDs recognized as GLM use the calibrated GLM prompt. The automatic fallback chains no longer include `glm-5.2`; manual GLM binding has less maintainer validation than Claude or Kimi.
