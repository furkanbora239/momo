...
export function createDelegateTaskPresentation(options: DelegateTaskPresentationOptions): DelegateTaskPresentation {
  ...
  **DO NOT provide both.** If category is provided, subagent_type is ignored.
  ...
  - run_in_background: Optional. Defaults to false (sync, waits) unless background_task.nonBlockingByDefault is true in config, in which case an omitted flag resolves to true (async). Set true=async (returns a background task ID like \`bg_...\` for \`background_output\`) ONLY for parallel exploration with 5+ independent queries.
