...
| `remove-deadcode/` | yes | Skill form of the `/remove-deadcode` command |
| `security-research/` | yes | Team Mode security research audit: 3 vulnerability hunters + 2 PoC engineers |
| `codex-qa/` | no | Isolated Codex Light QA: real `codex app-server` against an isolated `CODEX_HOME` + local mock model |
| `opencode-qa/` | no | opencode CLI/TUI/event-stream QA: hook-fired assertions via SSE, session DB inspection |
| `senpi-qa/` | no | Live Senpi adapter + task-engine QA against the real `senpi` binary |
