...
Execute a Prometheus work plan until every top-level checkbox is complete. This skill pairs with the harness's start-work continuation hook, which re-injects the next turn while `.omo/boulder.json` says this `senpi:<session_id>` still has unchecked plan work.
...
Write `.omo/boulder.json` before implementation starts. Prefix session ids with `senpi:` so the continuation hook can identify its own session.
...
- No unprefixed session ids in Boulder state. Sessions are always recorded as `senpi:<session_id>`.
